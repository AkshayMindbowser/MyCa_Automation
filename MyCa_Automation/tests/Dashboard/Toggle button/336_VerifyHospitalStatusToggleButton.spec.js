/**
 * Automated script for Qase case 336 - Verify hospital status toggle button
 */
...
(async () => {
  ...
  try {
    ...
    // toggle sequence: Inactive -> Active
    ...
    // attempt second toggle back to Active
    await tryClick(page.locator('button:has-text("Active")'));

    await page.waitForTimeout(1200);
    const after = await readRows();

    const screenshot = '336_case336.result.png';
    await page.screenshot({ path: screenshot, fullPage: true }).catch(() => {});

    if (!toggled) {
      console.log('❌ FAIL: Could not toggle status control.');
    ...
    if (same) {
      console.log('❌ FAIL: Toggle actions did not change hospital list.');
    ...
    console.log('✅ PASS: Hospital status toggle is functional (list changes observed).');
    console.log('Before count:', before.length, 'After count:', after.length);
    console.log('Screenshot saved:', screenshot);
    await browser.close();
    process.exit(0);

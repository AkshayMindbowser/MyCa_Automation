/**
 * Automated script for Qase case 500 - Verify status toggle functionality
 * Steps (from Qase):
 * 1) Click on Inactive option.
 * 2) Then click on Active option.
 * 3) Observe if data is changing according to the selection.
 */
...
(async () => {
  ...
  try {
    ...
    if (!toggled) try { await page.locator('[role="switch"], .toggle, .switch').first().click({ force: true }); toggled = true; } catch (e) {}

    await page.waitForTimeout(1200);
    const afterInactive = await readRows();

    // Step 2: Click Active
    await tryClick(page.locator('button:has-text("Active")'));
    await page.waitForTimeout(1200);
    const afterActive = await readRows();

    const screenshot = '500_case500.result.png';
    await page.screenshot({ path: screenshot, fullPage: true }).catch(() => {});

    if (!toggled) {
      console.log('❌ FAIL: Could not perform toggle actions (controls not found).');
      console.log('Screenshot saved:', screenshot);
      await browser.close();
      process.exit(1);
    }

    const changedAfterInactive = !(before.length === afterInactive.length && before.every((v, i) => v === afterInactive[i]));
    const changedAfterActive = !(afterInactive.length === afterActive.length && afterInactive.every((v, i) => v === afterActive[i]));

    if (changedAfterInactive || changedAfterActive) {
      console.log('✅ PASS: Data changed according to toggle selection.');
      console.log('Counts - before:', before.length, 'inactive:', afterInactive.length, 'active:', afterActive.length);
      console.log('Screenshot saved:', screenshot);
      await browser.close();
      process.exit(0);
    } else {
      console.log('❌ FAIL: No changes observed after toggling Inactive/Active.');
      console.log('Counts - before:', before.length, 'inactive:', afterInactive.length, 'active:', afterActive.length);
      console.log('Screenshot saved:', screenshot);
      await browser.close();
      process.exit(1);
